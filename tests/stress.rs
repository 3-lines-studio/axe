...
fn edit_fuzzy_matches_trailing_whitespace() {
    let dir = std::env::temp_dir().join(format!("axe-editfz-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");
...
fn edit_fuzzy_matches_smart_quotes_and_dashes() {
    let dir = std::env::temp_dir().join(format!("axe-editfq-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");
...
fn edit_fuzzy_preserves_untouched_lines() {
    let dir = std::env::temp_dir().join(format!("axe-editfp-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");
...
fn edit_mixed_exact_and_fuzzy_keeps_exact_line_bytes() {
    let dir = std::env::temp_dir().join(format!("axe-editmx-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");
...
fn edit_fuzzy_multiline_replaces_span() {
    let dir = std::env::temp_dir().join(format!("axe-editml-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");
    std::fs::write(&path, "one\ntwo\nthree\n").unwrap();
    let edit = axe::tools::edit();
...
fn edit_fuzzy_keeps_multibyte_chars_before_match() {
    let dir = std::env::temp_dir().join(format!("axe-editmb-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("f.txt");

...
use crate::session;
use crate::tui::{TuiConfig, build_tools};
...
impl App {
    ...
    fn draw(&mut self, frame: &mut ratatui::Frame) {
        ...
        let input_lines = self.input.lines().fold(0usize, |count, line| {
        ...
        let transcript = Paragraph::new(self.transcript())
        ...
        let mut status = if self.ctrl_c_armed.is_some() {
            ...
            format!(
                "{} · ↑{} ↓{}",
                self.cfg.model, self.session_input, self.session_output
            )
        };
        if !self.follow {
    ...
    fn transcript(&self) -> Text<'_> {
        let mut lines = vec![
            Line::from(vec![
                ...
                Span::styled(
                    format!(" v{} · Run /help for commands", crate::tui::VERSION),
                    Style::default().fg(Color::Gray),
                ),
            ]),
            Line::default(),
        ...
        for entry in &self.entries {
            match entry {
                ...
                Entry::Notice(text) => lines.push(Line::from(Span::styled(
                    text.clone(),
                    Style::default().fg(Color::Gray),
                ))),
            }

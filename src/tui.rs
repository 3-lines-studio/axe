...
const SLASH: &[SlashSpec] = &[
    ...
    SlashSpec {
        command: "/login",
        help: "/login",
        description: "set api key, base url and model in ~/.config/axe/config",
        category: "General",
    },
    SlashSpec {
        command: "/stats",
...
mod tests {
    ...
    fn write_login_config_roundtrip() {
        let dir = std::env::temp_dir().join(format!("axe-login-{}", std::process::id()));
    ...
    fn write_login_config_creates_file() {
        let dir = std::env::temp_dir().join(format!("axe-login-new-{}", std::process::id()));
        let root = dir.to_str().unwrap();
    ...
    fn login_wizard_writes_config() {
        let dir = std::env::temp_dir().join(format!("axe-login-wiz-{}", std::process::id()));
        let cfg = TuiConfig {
